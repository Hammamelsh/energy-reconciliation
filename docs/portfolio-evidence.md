...
| "Used dbt / Airflow / Spark / AWS" | Airflow, Spark and AWS: **untouched**. dbt: an **early slice only** (ANL-003 steps 0–3, 2026-09-08) — `dbt-core 1.12.4` / `dbt-duckdb 1.11.0`, a staging view, two policy models whose SQL is generated from `policy.py`, and the two tariff dimensions as Python models with verified `DECIMAL` types; all proven to return the same rows as the Python path. **The models that produce the charge are NOT in dbt**: no fact, no classification model, no publisher, no dbt test yet, and **the published scenario consumes nothing dbt builds**. The claim available is "began a dbt port, with the shared policy kept to one definition and typed decimal dimensions", never "built the pipeline in dbt". |
