...
| "Used dbt / Airflow / Spark / AWS" | None have been touched. The tariff SQL is written and tested, which is where a dbt port would start — not a copy-and-paste: model boundaries, `ref`/`source` wiring, the non-SQL steps, the rerun policy and the run identity all have to be decided. **ANL-003 is designed (`anl-003-dbt-design.md`, 2026-09-08) and not built**; a design document is not dbt experience and must not be presented as one. |
