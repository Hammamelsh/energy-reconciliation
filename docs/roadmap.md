...
**Step 5's publication core is built** (`publication.py`, design D5, `tests/test_publication.py`): immutable versioned warehouse files sealed read-only after validation, and a manifest replaced by atomic rename, so a build never locks the file a reader holds. `uv run build-candidate` creates and seals one; `uv run publication` promotes and rolls back. Sealing requires the candidate's **latest recorded build attempt** to have succeeded and its tables to digest as recorded (a versioned canonical digest, not a summary that can cancel), and every attempt records the candidate's full code and runtime identity, dbt included. A supported read contract (`tariff/reads.py`) now returns a validated context whose tariff relations and run identity are the dbt build's, so a consumer cannot silently read the Python scenario copied into the same file. The dashboard now reads a published version when asked to: the manifest is resolved once per rerun and every tab consumes that one sealed file, with an explicit unavailable state and no fallback. A published build can now be recorded as a **format-2 baseline** and genuinely rebuilt from its recorded inputs — re-ingested from the archive and rebuilt by dbt into a fresh destination — with every input, identity and logical output compared (measured on the real sample: 44 fields, 0 differ). **What does not exist: retention, any promotion control in the dashboard, and a format-1 replay of `4b3ee235d7ac` under current code (its recorded fingerprint predates later changes and that difference stands).** The scenario is still built entirely by
