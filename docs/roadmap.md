...
**Step 5's publication core is built** (`publication.py`, design D5, `tests/test_publication.py`): immutable versioned warehouse files sealed read-only after validation, and a manifest replaced by atomic rename, so a build never locks the file a reader holds. `uv run build-candidate` creates and seals one; `uv run publication` promotes and rolls back. **What does not exist: the dashboard switch and published-run identity integration.** The scenario is still built entirely by
