...
| Status | **Steps 0–3 implemented 2026-09-08. Steps 4–7 not started.** The staging view, the two policy models and both tariff dimensions build and run. **No fact and no publisher exists**, and the published scenario is still built entirely by `build-tariff-scenario`. |
...
| 4 | `int_classified_readings`, both facts, `schema.yml` tests, the six singular tests. **Also now required:** `unique` + `not_null` tests on `dim_tariff_band_schedule.schedule_label_naive` and a unique-combination test on `dim_tariff_price`, because CTAS drops the `PRIMARY KEY`/`NOT NULL` that `models.SCHEMA` declares (design D3). | `dbt build` on the demo warehouse passes every test; the reconciliation singular test fails when one exclusion flag is deliberately inverted (try it, then revert); the dimension uniqueness tests fail against a deliberately duplicated row | — |
