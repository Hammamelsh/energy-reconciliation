...
| Status | **Steps 0–4 implemented; step 5(a) — the publication core — implemented 2026-09-08; 5(b)–(e) and steps 6–7 not started.** `publication.py` provides resolve, finalise, publish, rollback, narrow recovery and a read-only inventory, exercised by 19 tests with real child processes. **Not yet wired in**: no `build-candidate` command, the dashboard still reads `data/warehouse/*.duckdb` directly, and the published scenario is still built by `build-tariff-scenario`. |
...
the design and the implementation agree, and do not exist yet. Until `build-candidate`
exists, a candidate is made by hand: a `COPY FROM DATABASE` snapshot (source attached
`READ_ONLY`) into `data/published/versions/`, then `run-dbt --database <that file>`.

| Stage | Proposed command | What it does | What you should see |
|---|---|---|---|
| build *(proposed)* | `uv run build-candidate --source data/warehouse/energy.duckdb` | snapshots the warehouse into `data/published/versions/cand-<run_id>.duckdb` (read-only attach; fails fast if ingestion holds the file), then runs `run-dbt` into it | `PASS=55` and a `validated` record naming the candidate's run_id, or a non-zero exit and a file that can never be promoted |
| validate **(real)** | `uv run publication finalise <file>` | refuses a `.wal`; requires **exactly one** successful `run-dbt` record; writes `<file>.validated.json` with the SHA-256 and run_id; makes the file read-only | `sealed <file> · run <run_id> · sha256 …` or a named refusal. A file whose build failed has no record and cannot be sealed |
...
## A guided local exercise for Hammam — steps 1, 4 and 5 are runnable now with `uv run publication`; 2, 3 and 6 wait for `build-candidate` and the dashboard switch
